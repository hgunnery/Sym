...
class SuggestionsTest < ApplicationSystemTestCase
  ...
  test "creating a Suggestion" do
    ...
    check "Admin suggested" if @suggestion.admin_suggested
    check "Completed" if @suggestion.completed
    fill_in "Description", with: @suggestion.description
    fill_in "Name", with: @suggestion.name
    check "Removed" if @suggestion.removed
    fill_in "Votes", with: @suggestion.votes
